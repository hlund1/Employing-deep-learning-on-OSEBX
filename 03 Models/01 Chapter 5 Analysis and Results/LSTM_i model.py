...
# Only constituents included in median_returns, NaN return values kept as NaN in binary_returns
median_returns = []
...
for i in range(0, len(return_window)):
    ...
    for s in range (0, X_train.shape[2]):
        classifier.fit(X_train[:,:,s:(s+1)], y_train[:,s], epochs = ep, batch_size = bs, validation_split = val_split, callbacks = [es], shuffle = False)
        pred_prob_s = classifier.predict(X_test[:,:,s:(s+1)])
        pred_class_s = classifier.predict_classes(X_test[:,:,s:(s+1)])
...
np.savetxt('04 Predictions/01 Chapter 5 Analysis and Results/01 prob/4 lstm_i prob.csv.csv', predicted_returns_prob, delimiter=",")
np.savetxt('04 Predictions/01 Chapter 5 Analysis and Results/01 class/4 lstm_i class.csv.csv', predicted_returns_class, delimiter=",")