...
pred_prob = pd.read_csv('04 Predictions/01 Chapter 5 Analysis and Results/01 prob/5 lstm_a prob.csv', header = None)       # "04/ Predictions/Before spread-adjusted returns/prob" as directory
pred_class = pd.read_csv('04 Predictions/01 Chapter 5 Analysis and Results/02 class/5 lstm_a class.csv', header = None)     # "04/ Predictions/Before spread-adjusted returns/prob" as directory
...
# Calculating binary returns and median returns
median_returns = []
...
# -------------------------------- Analysis I --------------------------------    
# running through portfolio creation
k = 5
tx = 0.00029
long = [100]
lidxx = []
...
for i in range(0, len(prob)):
    ...
    lidx = np.where(np.isin(a,e))[0].tolist()
    
    lidxx.append(lidx)  
    
    del_list = []
    for s in range(0, len(lidxx[i])):
        if np.isnan(real_return[i, lidxx[i][s]]) == True:
            del_list.append(lidxx[i][s])
    
    if len(del_list) != 0:
        for x in range(0, len(del_list)):
            lidxx[i][:] = (value for value in lidxx[i] if value != del_list[x])
...
# Concatinating identifier with number of trades
...
# Descriptive stats
sum(i > 0 for i in sorted_data['trades'])   # 192 out of 235 stocks are traded over the whole period

most_traded = sorted_data.nlargest(20, 'trades') 

# -------------------------------- Analysis II --------------------------------
# Creating empty table 
desc = np.empty(shape=(len(pred_prob)-240,241, 2))
desc[:,:,:] = np.nan

for i in range(240, len(pred_prob)):
    vec = list(list(np.where(binary_matrix[i, :]==1))[0])
    a = pred_prob[i,]
    b = a[~np.isnan(a)]
    e = b[np.argsort(b)[-10:]]  # top 10 probabilities
    
    # Adding chosen stocks to list
...
#plt.savefig('returns last 240 days selected stocks.png', bbox_inches='tight', dpi=1000)


'''
# analysing volume for top 20 stocks
volume = pd.read_csv('daily volume OSEBX_proc.csv', sep=',', header = None).iloc[:,1:]
